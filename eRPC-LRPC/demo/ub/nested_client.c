...
int main(int argc, char **argv)
{
	...
	if (lrpc_bind(&handle, device, 2, 1)) {
		fprintf(stderr, "UB nested client: bind: %s\n", strerror(errno));
		return 1;
	}
	...
	printf("UB_LRPC_NESTED_RESULT value=%llu rc=%d root_pid=%d "
	       "middle_pid=%llu leaf_pid=%llu cpu=%llu/%llu/%llu\n",
	...
	if (leaf_pid == call.shadow_pid ||
	    call.shadow_pid == (uint64_t)getpid() ||
	    call.caller_cpu_before != call.caller_cpu_in_service ||
	    call.caller_cpu_before != call.caller_cpu_after) {
		lrpc_close(&handle);
		return 2;
	}
	printf("UB_LRPC_NESTED_PASS calls=%zu switches=%zu depth=2 avg_ns=%llu\n",
	       samples, samples * 4,
