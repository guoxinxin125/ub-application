  ...
  if (likely(c->client.resp_msgbuf.get_data_size() > 0)) {
    ...
    switch (client_resp->resp_type) {
      ...
      case ClientRespType::kFailRedirect: {
        // 使用安全的 map 访问，避免 key 不存在时崩溃
        std::string leader_name;
        auto it = node_id_to_name_map.find(client_resp->leader_node_id);
        if (it != node_id_to_name_map.end()) {
          leader_name = it->second;
        } else {
          leader_name = "node_id=" + std::to_string(client_resp->leader_node_id);
        }
        
        printf(
            "smr: Client request to server %zu failed with code = "
            "redirect. Trying to change leader to %s.\n",
            c->client.leader_idx, leader_name.c_str());
        ...
        if (!success) {
          printf(
              ...
              leader_name.c_str(), c->client.leader_idx);
        }
...
void client_func(erpc::Nexus *nexus, AppContext *c) {
...
#ifdef ERPC_CXL
  // CXL 模式：Client 使用自己的 process_id 作为 RPC ID
  ...
  for (size_t i = 0; i < FLAGS_num_raft_servers; i++) {
...
#ifdef ERPC_CXL
    // CXL 模式：使用目标 Server 的 process_id 作为远程 RPC ID
    ...
    //        kAppClientRpcId, i, uri.c_str());
    c->conn_vec[i].session_num = c->rpc->create_session(uri, kAppClientRpcId);
#endif
