...
static constexpr size_t kAppValueSize = 128;
...
static constexpr size_t kAppNumaNode = 1;
...
inline size_t get_server_rpc_id() {
  return FLAGS_process_id;  // 每个 Server 使用自己的 process_id 作为 RPC ID
}

inline size_t get_client_rpc_id() {
  return FLAGS_num_raft_servers + (FLAGS_process_id - FLAGS_num_raft_servers);
}
...
#ifdef ERPC_CXL
  void* log_entry_buf = nullptr;  // CXL buffer to free after commit
#endif
};
...
class AppContext {
 public:
...
#ifdef ERPC_CXL
    // CXL 零拷贝相关：跟踪待释放的 entry 数据
    struct CXLEntryContext {
      void* data_ptr;
      size_t data_size;
      ...
      CXLEntryContext(void* ptr, size_t size, int expected)
          : data_ptr(ptr), data_size(size), expected_responses(expected),
            created_time(std::chrono::steady_clock::now()) {}
    };
    
    std::unordered_map<uint64_t, std::unique_ptr<CXLEntryContext>> cxl_entry_contexts;
    std::mutex cxl_contexts_mutex;
    
    // Commit latency 测量（更精确）
    std::unordered_map<uint64_t, uint64_t> entry_start_times;
    std::mutex entry_times_mutex;
    std::vector<double> commit_latency_samples;
#endif
  ...
  erpc::Rpc<SMR_TRANSPORT> *rpc = nullptr;
...
void sm_handler(int session_num, erpc::SmEventType sm_event_type,
  ...
  printf("sm_handler: session_num=%d, sm_event_type=%s, sm_err_type=%s\n",
         session_num, erpc::sm_event_type_str(sm_event_type).c_str(),
