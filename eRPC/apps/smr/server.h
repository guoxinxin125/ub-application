...
void init_raft(AppContext *c) {
  unsigned int seed = static_cast<unsigned int>(time(nullptr)) ^ 
                      (static_cast<unsigned int>(FLAGS_process_id) * 12345);
  srand(seed);
...
void client_req_handler(erpc::ReqHandle *req_handle, void *_context) {
  ...
  client_req_t *log_entry_appdata = nullptr;

#ifdef ERPC_CXL
  void* shared_buf = smr_cxl::alloc_entry_data(c->rpc, sizeof(client_req_t));
  if (shared_buf) {
    log_entry_appdata = static_cast<client_req_t*>(shared_buf);
    leader_sav.log_entry_buf = shared_buf; 
  } else {
    log_entry_appdata = c->server.log_entry_appdata_pool.alloc();
    leader_sav.log_entry_buf = nullptr;
  }
#else
  log_entry_appdata = c->server.log_entry_appdata_pool.alloc();
#endif
...
#ifdef ERPC_CXL
  {
    std::lock_guard<std::mutex> lock(c->server.entry_times_mutex);
    c->server.entry_start_times[static_cast<uint64_t>(ent.id)] = erpc::rdtsc();
  }
#endif
...
#ifdef ERPC_CXL
  // CXL 模式：每个节点使用自己的 process_id 作为 RPC ID
  // 因为 CXL 使用 RPC ID 作为共享内存队列的索引
  uint8_t my_rpc_id = static_cast<uint8_t>(FLAGS_process_id);
  c->rpc = new erpc::Rpc<SMR_TRANSPORT>(
      nexus, static_cast<void *>(c), my_rpc_id, sm_handler, kAppPhyPort);
#else
  // RDMA 模式：所有 Server 使用相同的 RPC ID
  c->rpc = new erpc::Rpc<SMR_TRANSPORT>(
...
#ifdef ERPC_CXL
    // CXL 模式：使用目标节点的 process_id 作为远程 RPC ID
    printf("smr: CXL mode - creating session with RPC ID %u for node %zu with uri %s.\n",
           static_cast<uint8_t>(i), i, uri.c_str());
    c->conn_vec[i].session_num = c->rpc->create_session(uri, static_cast<uint8_t>(i));
#else
    // RDMA 模式：所有 Server 使用相同的 RPC ID
    c->conn_vec[i].session_num = c->rpc->create_session(uri, kAppServerRpcId);
    printf("smr: RDMA mode - creating session with RPC ID %u for node %zu with uri %s.\n",
           kAppServerRpcId, i, uri.c_str());
#endif
...
void server_func(erpc::Nexus *nexus, AppContext *c) {
  ...
  while (ctrl_c_pressed == 0) {
    if (erpc::rdtsc() - loop_tsc > 1500000000ull) {
    ...
    if (c->server.need_snapshot) {
      raft_index_t num_snapshottable = raft_get_num_snapshottable_logs(c->server.raft);
      if (num_snapshottable > 1000) {  // 保留少量日志作为缓冲
        // printf("smr: Triggering log compaction, snapshottable logs = %ld\n", 
        //        static_cast<long>(num_snapshottable));
        
        // 1. 开始 Snapshot
        int e = raft_begin_snapshot(c->server.raft, 0);
        
        if (e == 0) {
          // 2. 结束 Snapshot (这会触发 log_poll 循环移除旧日志)
          raft_end_snapshot(c->server.raft);
          // printf("smr: Log compaction completed, remaining logs = %ld\n",
          //        raft_get_log_count(c->server.raft));
        } else {
          printf("smr: raft_begin_snapshot failed with error %d\n", e);
        }
      }
      c->server.need_snapshot = false;
    }
    ...
    if (commit_status == 1) {
      ...
      // 释放 CXL 共享内存
#ifdef ERPC_CXL
      if (leader_sav.log_entry_buf != nullptr) {
        smr_cxl::free_entry_data(c->rpc, leader_sav.log_entry_buf, sizeof(client_req_t));
        leader_sav.log_entry_buf = nullptr;
      }
#endif
