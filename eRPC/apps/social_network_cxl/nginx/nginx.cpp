...
void connect_sessions(ClientContext *c)
{
    ...
    my_assert(c->compose_post_session_num_ >=0);
    ...
    my_assert(c->user_timeline_session_num_ >=0);
    ...
    my_assert(c->home_timeline_session_num_ >=0);
    c->session_num_vec_.push_back(c->home_timeline_session_num_);

    c->backward_session_num_ = c->rpc_->create_session(load_balance_addr, get_remote_rpc_id(load_balance_addr));
...
void ping_handler(erpc::ReqHandle *req_handle, void *_context)
{
    ...
    // // printf("[nginx] ping_handler: PING received, pushing to forward_spsc_queue\n");
...
void ping_resp_handler(erpc::ReqHandle *req_handle, void *_context)
{
    ...
    ctx->rpc_->resize_msg_buffer(&req_handle->pre_resp_msgbuf_, sizeof(RPCMsgResp<PingRPCResp>));
        ...
        size_t c = ++send_ping_resp_num[req->req_common.req_number % 10000];
        // // printf("[nginx] ping_resp_handler: incremented for req_num=%zu to %zu\n", req->req_common.req_number, c);
        if(c == 3){
            // // printf("[nginx] ping_resp_handler: HIT 3! Pushing to backward_spsc_queue!\n");
...
void handler_ping_resp(ClientContext *ctx, const erpc::MsgBuffer &req_msgbuf)
{
    ...
    require_empty_msgbuf_slot(ctx->req_backward_msgbuf[slot],
                              "nginx.req_backward_msgbuf", slot);
...
void handler_common_req(ClientContext *ctx, const erpc::MsgBuffer &req_msgbuf)
{
    ...
    switch(static_cast<RPC_TYPE>(req_type)) {
        case RPC_TYPE::RPC_COMPOSE_POST_WRITE_REQ:
        ...
        case RPC_TYPE::RPC_HOME_TIMELINE_READ_REQ:
            ...
            break;
        default:
            fprintf(stderr, "illegal req type %ud\n", static_cast<unsigned >(req_type));
            exit(1);
    }
...
void server_thread_func(size_t thread_id, ServerContext *ctx, erpc::Nexus *nexus)
{
    ...
    while (true)
    {
        ...
        // printf("thread %zu: ping_req : %.2f, ping_resp : %.2f, cq_ut_ht_req : %.2f %.2f %.2f, cq_ut_ht_resp : %.2f %.2f %.2f \n", thread_id, ctx->stat_req_ping_tot / seconds, ctx->stat_req_ping_resp_tot / seconds, ctx->stat_req_compose_post_tot / seconds, ctx->stat_req_user_timeline_tot / seconds, ctx->stat_req_home_timeline_tot /seconds, ctx->stat_req_compose_post_resp_tot/seconds, ctx->stat_req_user_timeline_resp_tot/seconds, ctx->stat_req_home_timeline_resp_tot/seconds);
