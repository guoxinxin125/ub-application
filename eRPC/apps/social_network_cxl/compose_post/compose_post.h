...
class ReqState {
 public:
  explicit ReqState(uint32_t r_id, const PostData &input_post,
    ...
    original_text = local_post_string(post.text, post.text_length);
    creator.set_user_id(cxl_post_ptr->creator_user_id);
    creator.set_username(
  ...
  void send_first_step() {
    // unique id
    auto req_msgbuf1 =
        rpc_->alloc_msg_buffer_or_die(sizeof(RPCMsgReq<UniqueIDReq>));
    PostStorageWriteCXLReq request{cxl_post_ptr->post_id, *cxl_post_ptr};
    new (req_msgbuf1.buf_)
        RPCMsgReq<UniqueIDReq>(RPC_TYPE::RPC_UNIQUE_ID, req_id, {0});
    consumer_mpmc_queue->push(req_msgbuf1);

    // user
    auto req_msgbuf2 = rpc_->alloc_msg_buffer_or_die(
    ...
    // user_mention
    auto req_msgbuf3 = rpc_->alloc_msg_buffer_or_die(
        sizeof(RPCMsgReq<PostStorageWriteCXLReq>));
    ...
    // url_shorten
    auto req_msgbuf4 = rpc_->alloc_msg_buffer_or_die(
        sizeof(RPCMsgReq<PostStorageWriteCXLReq>));
  ...
  void send_second_step() {
    ...
    req_msgbuf =
        rpc_->alloc_msg_buffer_or_die(sizeof(RPCMsgReq<UserTimeLineWriteReq>));
    new (req_msgbuf.buf_) RPCMsgReq<UserTimeLineWriteReq>(
        RPC_TYPE::RPC_USER_TIMELINE_WRITE_REQ, req_id,
        {cxl_post_ptr->post_id, creator.user_id(), cxl_post_ptr->timestamp});
    consumer_mpmc_queue->push(req_msgbuf);
  }
  erpc::MsgBuffer generate_resp_msg() {
    erpc::MsgBuffer resp_buf =
        rpc_->alloc_msg_buffer_or_die(sizeof(RPCMsgReq<CommonRPCReq>));
    new (resp_buf.buf_) RPCMsgReq<CommonRPCReq>(
        RPC_TYPE::RPC_COMPOSE_POST_WRITE_RESP, req_id, {0});
    return resp_buf;
  }
...
class ClientContext : public BasicContext {
 public:
  ClientContext(size_t cid, size_t sid, size_t rid)
      : client_id_(cid), server_sender_id_(sid), server_receiver_id_(rid) {
 ...
 public:
  AppContext() {
    ...
    for (size_t i = 0; i < FLAGS_client_num; i++) {
      client_contexts_.push_back(new ClientContext(
          i, (i % FLAGS_server_num) + kAppMaxRPC, i % FLAGS_server_num));
    }
  ...
  [[maybe_unused]] [[nodiscard]] bool write_latency_and_reset(
...
void compose_post_write_and_create(void *buf_, ReqStateStore *store,
                                   MPMC_QUEUE *forward_queue, AppRpc *c_rpc) {
  ...
  ReqState *req_state =
      new ReqState(req->req_common.req_number, post, forward_queue, c_rpc);
