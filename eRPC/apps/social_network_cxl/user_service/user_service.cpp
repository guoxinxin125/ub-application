...
void compose_creator_with_user_id_handler(erpc::ReqHandle *req_handler,
                                          void *_context) {
  auto *ctx = static_cast<ServerContext *>(_context);
  ...
  [[maybe_unused]] std::string username =
      local_post_string(post.creator_username, post.creator_username_length);

  size_t resp_data_length = 0;

  new (req_handler->pre_resp_msgbuf_.buf_) RPCMsgResp<CommonRPCResp>(
      req->req_common.type, req->req_common.req_number, 0, {resp_data_length});

  ctx->rpc_->resize_msg_buffer(
      &req_handler->pre_resp_msgbuf_,
      sizeof(RPCMsgResp<CommonRPCResp>) + resp_data_length);
...
void handler_ping_resp(ClientContext *ctx, const erpc::MsgBuffer &req_msgbuf) {
  ...
  require_empty_msgbuf_slot(ctx->req_backward_msgbuf[slot],
                            "user_service.req_backward_msgbuf", slot);
...
void server_thread_func(size_t thread_id, ServerContext *ctx,
  ...
  while (true) {
    ...
    // ctx->stat_req_compose_creator_with_user_id_tot / seconds);
...
void leader_thread_func() {
  ...
  nexus.register_req_func(
      ...
      compose_creator_with_user_id_handler);
