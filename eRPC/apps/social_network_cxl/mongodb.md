...
# 下载并安装到 $HOME/.local（修改版本号为你想要的）
mkdir -p $HOME/src $HOME/.local
cd $HOME/src
wget https://github.com/Kitware/CMake/releases/download/v3.26.4/cmake-3.26.4-linux-x86_64.sh
sh cmake-3.26.4-linux-x86_64.sh --skip-license --prefix=$HOME/.local

# 生效当前会话
export PATH=$HOME/.local/bin:$PATH
cmake --version
3. gflags下载
# 下载并编�?gflags �?$HOME/.local（无 sudo�?
...
cmake -DENABLE_AUTOMATIC_INIT_AND_CLEANUP=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$HOME/.local -DENABLE_TESTS=OFF ../
make -j4
make install
...
cmake .. -DCMAKE_PREFIX_PATH=$HOME/.local -Dgflags_DIR=$HOME/.local/lib/cmake/gflags   -DSOCIAL_NETWORK_TYPE=erpc -DTRANSPORT=infiniband   -DProtobuf_INCLUDE_DIR=$HOME/.local/include   -DProtobuf_LIBRARIES=$HOME/.local/lib/libprotobuf.a   -DProtobuf_PROTOC_EXECUTABLE=$HOME/.local/bin/protoc
make -j4
7. 运行
cd DmRPC
