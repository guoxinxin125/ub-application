...
void user_timeline_write_req_handler(erpc::ReqHandle *req_handle, void *_context)
{
    ...
    if (unlikely(!ctx->mongodb_init_finished.load(
        ...
        throw std::runtime_error("user_timeline: MongoDB is not initialized");
    }
...
void handler_user_timeline_write_resp(ClientContext *ctx, const erpc::MsgBuffer &req_msgbuf)
{
    ...
    ctx->rpc_->enqueue_request(ctx->compose_post_session_number, static_cast<uint8_t>(RPC_TYPE::RPC_USER_TIMELINE_WRITE_RESP),
                               &ctx->req_backward_msgbuf[slot], &resp_msgbuf,
                               callback_user_timeline_write_resp, reinterpret_cast<void *>(slot));
}
...
void handler_user_timeline_read_resp(ClientContext *ctx, const erpc::MsgBuffer &req_msgbuf)
{
    ...
    ctx->rpc_->enqueue_request(ctx->nginx_session_number, static_cast<uint8_t>(RPC_TYPE::RPC_USER_TIMELINE_READ_RESP),
                               &ctx->req_backward_msgbuf[slot], &resp_msgbuf,
                               callback_user_timeline_read_resp, reinterpret_cast<void *>(slot));
...
void callback_post_storage_read_req(void *_context, void *_tag)
{
    ...
    handler_user_timeline_read_resp(ctx, result);
    ...
    while (true)
    {
        ...
        for (size_t i = 0;
            ...
            if (sn_profile::enabled()) {
                auto *forward_req = reinterpret_cast<CommonReq *>(req_msg.buf_);
                const size_t slot = forward_req->req_number % kAppMaxBuffer;
                sn_profile::record(
        ...
        for (size_t i = 0;
            ...
            switch (req->type) {
                case RPC_TYPE::RPC_PING_RESP:
                    ...
                    break;
                case RPC_TYPE::RPC_USER_TIMELINE_WRITE_RESP:
                    handler_user_timeline_write_resp(ctx, req_msg);
                    break;
                case RPC_TYPE::RPC_USER_TIMELINE_READ_RESP:
                    handler_user_timeline_read_resp(ctx, req_msg);
                    break;
                default:
                    my_assert(false);
            }
    ...
    while (true)
    {
        ...
        for (size_t i = 0;
            ...
            if(req->type == RPC_TYPE::RPC_USER_TIMELINE_READ_REQ){
                if (sn_profile::enabled()) {
                    const size_t slot = req->req_number % kAppMaxBuffer;
                    sn_profile::record(
                ...
                read_post_details(req_msg.buf_,rpc_,consumer_fwd,consumer_back,forward_queue_start_ns);
                ...
                auto *local_req = reinterpret_cast<CommonReq *>(local.buf_);
                local_req->type = RPC_TYPE::RPC_USER_TIMELINE_READ_RESP;
                consumer_back->push(local);
            } else {
                erpc::MsgBuffer local = prepare_forward_msgbuf(rpc_, req_msg);
                auto *local_req = reinterpret_cast<CommonReq *>(local.buf_);
                local_req->type = RPC_TYPE::RPC_PING_RESP;
                consumer_back->push(local);
            }
        }
        if (ctrl_c_pressed == 1)
