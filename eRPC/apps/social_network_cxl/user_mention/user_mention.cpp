...
void ping_handler(erpc::ReqHandle *req_handle, void *_context) {
  ...
  if (ctx->mongodb_init_finished) {
    //         ctx->forward_spsc_queue->push(clone_msgbuf(ctx->rpc_,
    //         *req_msgbuf));
...
void user_mention_handler(erpc::ReqHandle *req_handler, void *_context) {
  ...
  while (it1 != end) {
    ...
    if (user_mention_map.find(name) != user_mention_map.end()) {
      ...
      if (result.count == SN_MAX_MENTIONS)
        throw std::length_error("PostData: too many resolved mentions");
      const size_t i = result.count++;
      result.user_ids[i] = target_user_mention->user_id();
      result.username_lengths[i] =
          set_post_string(result.usernames[i], target_user_mention->username());
    } else {
      fprintf(stderr, "Warning: don't find user: %s\n", name.c_str());
    }
    it1++;
  }
  new (req_handler->pre_resp_msgbuf_.buf_) RPCMsgResp<UserMentionRPCResp>(
      req->req_common.type, req->req_common.req_number, 0, result);

  ctx->rpc_->resize_msg_buffer(&req_handler->pre_resp_msgbuf_,
...
void handler_ping_resp(ClientContext *ctx, const erpc::MsgBuffer &req_msgbuf) {
  ...
  require_empty_msgbuf_slot(ctx->req_backward_msgbuf[slot],
                            "user_mention.req_backward_msgbuf", slot);
...
void server_thread_func(size_t thread_id, ServerContext *ctx,
  ...
  while (true) {
    ...
    // ctx->stat_req_user_mention_tot / seconds);
...
void mongodb_init(AppContext *ctx) {
  mongodb_client_pool =
      init_mongodb_client_pool(config_json_all, "user", mongodb_conns_num);
