...
void connect_sessions(ClientContext *c) {
  ...
  c->backward_session_num_ = c->rpc_->create_session(
...
void url_shorten_handler(erpc::ReqHandle *req_handler, void *_context) {
  ...
  my_assert(
      req_msgbuf->get_data_size() == sizeof(RPCMsgReq<PostStorageWriteCXLReq>),
      "data size not match");

  PostData post;
  std::memcpy(&post, &req->req_control.post, sizeof(post));
  validate_post_data(post);
  std::string original_text = local_post_string(post.text, post.text_length);
  UrlShortenRPCResp result{};

  static const std::regex e("(http://|https://)([a-zA-Z0-9_!~*'().&=+$%-]+)");
  std::sregex_iterator it2(original_text.begin(), original_text.end(), e);
  std::sregex_iterator end;
  while (it2 != end) {
    std::string url = (*it2).str();
    std::string shortened = HOSTNAME + generate_random_string();
    if (result.count == SN_MAX_URLS)
      throw std::length_error("PostData: too many resolved URLs");
    PostUrlData &entry = result.urls[result.count++];
    entry.shortened_length = set_post_string(entry.shortened, shortened);
    entry.expanded_length = set_post_string(entry.expanded, url);
    it2++;
  }
  // This response may exceed a transport's preallocated response capacity.
  req_handler->dyn_resp_msgbuf_ =
      ctx->rpc_->alloc_msg_buffer_or_die(sizeof(RPCMsgResp<UrlShortenRPCResp>));
  new (req_handler->dyn_resp_msgbuf_.buf_) RPCMsgResp<UrlShortenRPCResp>(
      req->req_common.type, req->req_common.req_number, 0, result);
...
void handler_ping_resp(ClientContext *ctx, const erpc::MsgBuffer &req_msgbuf) {
  ...
  ctx->rpc_->enqueue_request(
      ctx->backward_session_num_, static_cast<uint8_t>(RPC_TYPE::RPC_PING_RESP),
...
void client_thread_func(size_t thread_id, ClientContext *ctx,
  ...
  while (true) {
    ...
    for (size_t i = 0;
         i < kAppMaxBuffer && ctx->backward_spsc_queue->try_pop(req_msg); i++) {
...
void server_thread_func(size_t thread_id, ServerContext *ctx,
  ...
  while (true) {
    ...
    // thread_id, ctx->stat_req_ping_tot / seconds,
    // ctx->stat_req_url_shorten_tot / seconds);
...
void worker_thread_func(size_t thread_id, SPSC_QUEUE *producer,
                        SPSC_QUEUE *consumer, AppRpc *rpc_,
                        AppRpc *server_rpc_) {
...
void leader_thread_func() {
  ...
  servers[0] =
      std::thread(server_thread_func, 0, context->server_contexts_[0], &nexus);
  sleep(2);
  erpc::bind_to_core(
      servers[0], FLAGS_numa_server_node,
  ...
  for (size_t i = 0; i < FLAGS_client_num; i++) {
    ...
    workers[i] = std::thread(
        worker_thread_func, i, context->client_contexts_[i]->forward_spsc_queue,
        context->client_contexts_[i]->backward_spsc_queue,
        context->client_contexts_[i]->rpc_, context->server_contexts_[i]->rpc_);
    ////    uint64_t worker_offset = FLAGS_worker_bind_core_offset == UINT64_MAX
    ///? FLAGS_bind_core_offset : FLAGS_worker_bind_core_offset; /
    ///erpc::bind_to_core(workers[i], FLAGS_numa_worker_node,
    ///get_bind_core(FLAGS_numa_worker_node) + worker_offset);
    erpc::bind_to_core(
