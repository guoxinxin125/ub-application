...
void unique_id_handler(erpc::ReqHandle *req_handler, void *_context)
{
    ...
    my_assert(req_msgbuf->get_data_size() == sizeof(RPCMsgReq<UniqueIDReq>), "data size not match");
...
void handler_ping_resp(ClientContext *ctx, const erpc::MsgBuffer &req_msgbuf)
{
    ...
    require_empty_msgbuf_slot(ctx->req_backward_msgbuf[slot],
                              "unique_id.req_backward_msgbuf", slot);
...
void server_thread_func(size_t thread_id, ServerContext *ctx, erpc::Nexus *nexus)
{
    ...
    while (true)
    {
        ...
        // printf("thread %zu: ping_req : %.2f, uniqueid_req : %.2f \n", thread_id, ctx->stat_req_ping_tot / seconds, ctx->stat_req_unique_id_tot / seconds);
