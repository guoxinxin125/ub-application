...
class QueueStore {
 public:
  QueueStore() {
    spsc_queue =
        new atomic_queue::AtomicQueueB2<REQ_MSG, std::allocator<REQ_MSG>, true,
                                        false, false>(kAppMaxBuffer);
    resp_spsc_queue =
        new atomic_queue::AtomicQueueB2<RESP_MSG, std::allocator<RESP_MSG>,
                                        true, false, false>(kAppMaxBuffer);
    req_id_ = 1;
    rpc_type_arr.insert(rpc_type_arr.end(), user_num_,
                        RPC_TYPE::RPC_USER_TIMELINE_READ_REQ);
    rpc_type_arr.insert(rpc_type_arr.end(), home_num_,
                        RPC_TYPE::RPC_HOME_TIMELINE_READ_REQ);
    rpc_type_arr.insert(rpc_type_arr.end(), write_num_,
                        RPC_TYPE::RPC_COMPOSE_POST_WRITE_REQ);
  }
  ~QueueStore() {
    delete spsc_queue;
    delete resp_spsc_queue;
  }
  void PushPingReq() { spsc_queue->push(REQ_MSG{0, RPC_TYPE::RPC_PING}); }
  void PushParamReq() {
  ...
  void PushNextReq() {
    ...
    mutex.unlock();
    spsc_queue->push(
        REQ_MSG{now_req_id, rpc_type_arr[now_req_id % rpc_type_arr.size()]});
  }
  void PushReq(REQ_MSG msg) { spsc_queue->push(msg); }
  void PushResp(RESP_MSG msg) { resp_spsc_queue->push(msg); }
  REQ_MSG PopReq() { return spsc_queue->pop(); }
  bool TryPopReq(REQ_MSG &msg) { return spsc_queue->try_pop(msg); }
  unsigned GetReqSize() { return spsc_queue->was_size(); }
  RESP_MSG PopResp() { return resp_spsc_queue->pop(); }
  unsigned GetRespSize() { return resp_spsc_queue->was_size(); }
  spinlock_mutex mutex;
  uint32_t req_id_;
  atomic_queue::AtomicQueueB2<REQ_MSG, std::allocator<REQ_MSG>, true, false,
                              false> *spsc_queue;
  atomic_queue::AtomicQueueB2<RESP_MSG, std::allocator<RESP_MSG>, true, false,
                              false> *resp_spsc_queue{};
...
using READER_QUEUE = atomic_queue::AtomicQueueB2<
    ReaderHandler *, std::allocator<ReaderHandler *>, true, false, false>;
std::vector<READER_QUEUE *> reader_queues;

class ClientContext : public BasicContext {
 public:
  ClientContext(size_t cid, size_t sid)
      : client_id_(cid), server_sender_id_(sid) {
...
void generate_compose_post_req_msgbuf(AppRpc *rpc_,
                                      SPSC_QUEUE *consumer_queue) {
  for (int64_t i = 0; i < generate_data_num; i++) {
    ...
    post.creator_user_id = i % 962;  // Ensure creator exists
    {
      ...
      post.creator_username_length = set_post_string(post.creator_username, u);
    }
    post.req_id = i;
    std::string text = generate_random_string(500);
    ...
    // fill text
    post.text_length = set_post_string(post.text, text);
    // media
    for (size_t tmp = 0; tmp < 10; tmp++) {
      post.media_ids[tmp] = rand();
      const char *mt = "png";
      post.media_type_lengths[tmp] = set_post_string(post.media_types[tmp], mt);
      post.media_count++;
    }
    post.post_type =
        static_cast<int32_t>(social_network::PostType::POST_TYPE_POST);
    auto req_msgbuf = rpc_->alloc_msg_buffer_or_die(
...
void generate_user_timeline_req_msgbuf(AppRpc *rpc_,
                                       SPSC_QUEUE *consumer_queue) {
  for (int64_t i = 0; i < generate_data_num; i++) {
    auto req_msgbuf =
        rpc_->alloc_msg_buffer_or_die(sizeof(RPCMsgReq<UserTimeLineReq>));
    new (req_msgbuf.buf_) RPCMsgReq<UserTimeLineReq>(
        RPC_TYPE::RPC_USER_TIMELINE_READ_REQ, 0, {0, i % 962, 0, 1});
    consumer_queue->push(req_msgbuf);
  }
}

void generate_home_timeline_req_msgbuf(AppRpc *rpc_,
                                       SPSC_QUEUE *consumer_queue) {
  for (int64_t i = 0; i < generate_data_num; i++) {
    auto req_msgbuf =
        rpc_->alloc_msg_buffer_or_die(sizeof(RPCMsgReq<HomeTimeLineReq>));
    new (req_msgbuf.buf_) RPCMsgReq<HomeTimeLineReq>(
        RPC_TYPE::RPC_HOME_TIMELINE_READ_REQ, 0, {0, i, 0, 1});
    consumer_queue->push(req_msgbuf);
  }
}
