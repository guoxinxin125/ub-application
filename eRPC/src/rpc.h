...
class Rpc {
 ...
 public:
  ...
  inline MsgBuffer alloc_msg_buffer(size_t max_data_size) {
    ...
    if (std::is_same<TTr, CTransport>::value) {
        ...
        // 从共享内存分配器分配（关键修改）
        buffer = shared_transport->alloc_shared_buffer(
            max_data_size + (max_num_pkts * sizeof(pkthdr_t)));

        if (unlikely(buffer.buf_ == nullptr)) {
            ERPC_WARN("CXL: Failed to allocate shared memory buffer\n");
            MsgBuffer msg_buffer;
            msg_buffer.buf_ = nullptr;
            return msg_buffer;
        }

        // ERPC_INFO("CXL: Allocated msg buffer in shared memory, size %zu\n",
        //           max_data_size);
    } else
#endif
    {
        lock_cond(&huge_alloc_lock_);
        buffer = huge_alloc_->alloc(
            max_data_size + (max_num_pkts * sizeof(pkthdr_t)));
  ...
  inline void free_msg_buffer(MsgBuffer msg_buffer) {
    ...
    if (std::is_same<TTr, CTransport>::value) {
        CTransport* shared_transport = static_cast<CTransport*>(transport_);
        shared_transport->free_shared_buffer(msg_buffer.buffer_);
        return;
    }
#endif
  ...
  TTr* get_transport() { return transport_; }
 ...
 private:
  ...
  inline const uint8_t *get_pkt_payload_ptr(const pkthdr_t *pkthdr) const {
#if defined(ERPC_CXL) || defined(ERPC_UB)
    if (std::is_same<TTr, CTransport>::value) {
      CTransport *shared_transport = static_cast<CTransport *>(transport_);
      const uint8_t *payload = shared_transport->get_rx_payload(pkthdr);
      if (payload != nullptr) return payload;
    }
#endif
    return reinterpret_cast<const uint8_t *>(pkthdr + 1);
  }
  ...
  inline void copy_data_to_msgbuf(MsgBuffer *msgbuf, size_t pkt_idx,
    ...
    if (std::is_same<TTr, CTransport>::value) {
      ...
      if (pkthdr->msg_size_ > 0) {
        memcpy(msgbuf->buf_, get_pkt_payload_ptr(pkthdr), pkthdr->msg_size_);
      }
      return;
    }
#endif
    ...
    memcpy(&msgbuf->buf_[offset], get_pkt_payload_ptr(pkthdr), to_copy);
  }
...
#ifdef ERPC_CXL
extern template class Rpc<CXLTransport>;
#elif defined(ERPC_UB)
extern template class Rpc<UBTransport>;
#endif
