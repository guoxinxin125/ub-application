...
static const size_t k_pkt_hdr_magic_bits =
    128 -
    (kHeadroomHackBits + 8 + kMsgSizeBits + 16 + 2 + kPktNumBits +
     kReqNumBits
...
#else
static_assert(k_pkt_hdr_magic_bits == 5, "");  // Just to keep track
#endif
...
#if defined(ERPC_CXL) || defined(ERPC_UB)
  uint64_t is_zerocopy_ : 1;  ///< CXL zero-copy payload flag
#endif
...
#if defined(ERPC_CXL) || defined(ERPC_UB)
        << ", zerocopy " << std::to_string(is_zerocopy_)
#endif
        << "]";
