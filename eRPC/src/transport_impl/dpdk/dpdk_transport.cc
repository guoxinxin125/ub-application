...
DpdkTransport::DpdkTransport(uint16_t sm_udp_port, uint8_t rpc_id,
...

  {
    ...
    if (g_dpdk_initialized) {
      ...
      // 为每个进程生成唯一的文件前缀，避免多进程冲突
    std::string file_prefix = "erpc_" + std::to_string(getpid()) + "_" + std::to_string(rpc_id);
    
    // clang-format off
    const char *rte_argv[] = {
        "-c",            "0x0",
        "-n",            "6",  // Memory channels
        "-m",            "512", // Max memory in megabytes
        "--proc-type",   "primary",  // 强制为 primary
        "--file-prefix", file_prefix.c_str(),  // 使用唯一的文件前缀
        "--log-level",   "3",
        nullptr};
    // clang-format on
      ...
      dpdk_proc_type_ = DpdkProcType::kPrimary;
  ...
  ERPC_WARN(
      "DpdkTransport created for Rpc ID %u, queue %zu, datapath UDP port %u, physical port %u\n",
      rpc_id, qp_id_, rx_flow_udp_port_, phy_port_);
}
...
void DpdkTransport::resolve_phy_port() {
  ...
  rt_assert(drv_name == "net_mlx4" or drv_name == "net_mlx5" or
                drv_name == "mlx5_pci" or drv_name == "net_e1000_igb" or
                drv_name == "net_igb" or drv_name == "net_ixgbe",
            "eRPC supports mlx4, mlx5, and Intel e1000/igb/ixgbe devices with DPDK");
  ...
  } else if (drv_name == "net_e1000_igb" or drv_name == "net_igb") {
    // Intel I350 网卡通常支持较少的队列
    resolve_.reta_size_ = std::max(dev_info.reta_size, static_cast<uint16_t>(8));
  } else {
  ...
  if (dpdk_proc_type_ == DpdkProcType::kPrimary) {
    ...
    if (link.link_status != ETH_LINK_UP) {
      ERPC_WARN("Port %u is down, but continuing anyway for Intel I350 compatibility.\n", 
                phy_port_);
      link.link_speed = 1000;  // 设置默认速度为 1 Gbps
    }
  } else {
  ...
  if (link.link_speed != ETH_SPEED_NUM_NONE) {
    ...
    rt_assert(link.link_speed >= 1000, "Link too slow");
    resolve_.bandwidth_ =
        static_cast<size_t>(link.link_speed) * 1000 * 1000 / 8.0;
  } else {
    ERPC_WARN(
        "Port %u bandwidth not reported by DPDK. Using default 1 Gbps.\n",
        phy_port_);
    link.link_speed = 1000;
    resolve_.bandwidth_ = 1.0 * (1000 * 1000 * 1000) / 8.0;
  }
