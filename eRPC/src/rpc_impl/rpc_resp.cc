...
void Rpc<TTr>::enqueue_response(ReqHandle *req_handle, MsgBuffer *resp_msgbuf) {
  ...
  if (std::is_same<TTr, CTransport>::value) {
    resp_msgbuf->set_detached_pkthdr(&sslot->tx_pkthdr_);
  }
#endif
...
#if defined(ERPC_CXL) || defined(ERPC_UB)
  // For CXL Transport: Check if the response buffer is in shared memory for
  // zerocopy.
  if (std::is_same<TTr, CTransport>::value) {
    CTransport *shared_transport = static_cast<CTransport *>(transport_);
    resp_pkthdr_0->is_zerocopy_ =
        shared_transport->is_in_shared_memory(resp_msgbuf->buffer_.buf_);
  }
#endif
...
void Rpc<TTr>::process_resp_one_st(SSlot *sslot, const pkthdr_t *pkthdr,
  ...
  // printf("eRPC RX process_resp_one_st: rpc_id=%u, dest_session_num=%u, req_num=%lu, sslot_req_num=%lu, pkt_num=%lu, ci.num_rx_=%lu, ci.num_tx_=%lu\n", rpc_id_, pkthdr->dest_session_num_, pkthdr->req_num_, sslot->cur_req_num_, pkthdr->pkt_num_, sslot->client_info_.num_rx_, sslot->client_info_.num_tx_);
  // Handle reordering 
  ...
  bool is_single_pkt_resp = pkthdr->msg_size_ <= TTr::kMaxDataPerPkt;
#if defined(ERPC_CXL) || defined(ERPC_UB)
  if (std::is_same<TTr, CTransport>::value) is_single_pkt_resp = true;
#endif

  // Special handling for single-packet responses
  if (likely(is_single_pkt_resp)) {
#if defined(ERPC_CXL) || defined(ERPC_UB)
    const uint8_t *payload = get_pkt_payload_ptr(pkthdr);


    if (std::is_same<TTr, CTransport>::value && resp_msgbuf->buf_ == nullptr &&
        pkthdr->is_zerocopy_ != 0) {
      *resp_msgbuf = MsgBuffer::make_detached(
          const_cast<pkthdr_t *>(pkthdr), const_cast<uint8_t *>(payload),
          pkthdr->msg_size_);
      resp_msgbuf->own_pkthdr_copy();
      if (payload != nullptr && pkthdr->msg_size_ > 0) {
        resp_msgbuf->buffer_ =
            Buffer(const_cast<uint8_t *>(payload) - sizeof(pkthdr_t),
                   pkthdr->msg_size_ + sizeof(pkthdr_t), 0);
      }
    } else
    {
      if (resp_msgbuf->buf_ == nullptr) {
        if (pkthdr->msg_size_ > 0) {
          *resp_msgbuf = alloc_msg_buffer(pkthdr->msg_size_);
        } else {
          resp_msgbuf->max_data_size_ = 0;
          resp_msgbuf->data_size_ = 0;
          resp_msgbuf->max_num_pkts_ = 1;
          resp_msgbuf->num_pkts_ = 1;
        }
      }
      if (resp_msgbuf->buf_ != nullptr) {
        ...
        memcpy(resp_msgbuf->buf_, payload, pkthdr->msg_size_);
      }

      if (std::is_same<TTr, CTransport>::value && payload != nullptr &&
          pkthdr->msg_size_ > 0) {
        CTransport *shared_transport = static_cast<CTransport *>(transport_);
        const uint8_t *buffer_start = pkthdr->is_zerocopy_ != 0
                                          ? payload - sizeof(pkthdr_t)
    ...
    if (ci.num_rx_ != wire_pkts(req_msgbuf, resp_msgbuf)) return;
      // printf("eRPC RX process_resp_one_st multi-packet: ci.num_rx_=%lu, pkt_idx=%zu\n", ci.num_rx_, pkt_idx);
    // Else fall through to invoke continuation
  }

  // printf("eRPC RX process_resp_one_st: FINISHED RESPONSE. Invoking cont_func=%p, sslot_req_num=%lu\n", (void*)sslot->client_info_.cont_func_, sslot->cur_req_num_);
