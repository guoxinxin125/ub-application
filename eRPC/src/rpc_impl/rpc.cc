...
// #endif
  ...
  size_t initialized_ctrl_msgbufs = 0;
  for (MsgBuffer &ctrl_msgbuf : ctrl_msgbufs_) {
    ...
    if (ctrl_msgbuf.buf_ == nullptr) {
      for (size_t i = 0; i < initialized_ctrl_msgbufs; ++i) {
        free_msg_buffer(ctrl_msgbufs_[i]);
        ctrl_msgbufs_[i] = MsgBuffer();
      }
      delete huge_alloc_;
      huge_alloc_ = nullptr;
      delete transport_;
...
Rpc<TTr>::~Rpc() {
  ...
  // Shared-memory transports cannot rely on HugeAlloc destruction to reclaim
  // request/response buffers. Release every server-owned MsgBuffer before the
  ...
  for (Session *session : session_vec_) {
    ...
    if (std::is_same<TTr, CTransport>::value && session->is_server()) {
      for (SSlot &sslot : session->sslot_arr_) {
        bury_req_msgbuf_server_st(&sslot);
        bury_resp_msgbuf_server_st(&sslot);
        if (sslot.pre_resp_msgbuf_.is_dynamic()) {
          free_msg_buffer(sslot.pre_resp_msgbuf_);
  ...
  if (std::is_same<TTr, CTransport>::value) {
    for (MsgBuffer &ctrl_msgbuf : ctrl_msgbufs_) {
      if (!ctrl_msgbuf.is_dynamic()) continue;
      free_msg_buffer(ctrl_msgbuf);
