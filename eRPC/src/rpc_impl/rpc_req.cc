...
void Rpc<TTr>::enqueue_request(int session_num, uint8_t req_type,
  ...
  if (std::is_same<TTr, CTransport>::value) {
    req_msgbuf->set_detached_pkthdr(&sslot.tx_pkthdr_);
  }
#endif
...
#if defined(ERPC_CXL) || defined(ERPC_UB)
  // For CXL Transport: Check if the buffer is in shared memory for zerocopy
  if (std::is_same<TTr, CTransport>::value) {
    CTransport *shared_transport = static_cast<CTransport *>(transport_);
    pkthdr_0->is_zerocopy_ =
        shared_transport->is_in_shared_memory(req_msgbuf->buffer_.buf_);
  }
#endif
...
void Rpc<TTr>::process_small_req_st(SSlot *sslot, pkthdr_t *pkthdr) {
...
#if defined(ERPC_CXL) || defined(ERPC_UB)
        ERPC_WARN("%s: Re-sending response.\n", issue_msg);
#else
...
#if defined(ERPC_CXL) || defined(ERPC_UB)
        ERPC_WARN("%s: Response not available yet. Dropping.\n", issue_msg);
#else
  ...
  if (req_func.req_func_ == nullptr) {
    printf("FATAL: Unregistered req_type_=%u received! rpc_id=%u\n", pkthdr->req_type_, rpc_id_);
    exit(1);
  }
#endif
  ...
  if (likely(!req_func.is_background())) {
    if (kZeroCopyRX) {
...
#if defined(ERPC_CXL) || defined(ERPC_UB)
      const uint8_t *payload = get_pkt_payload_ptr(pkthdr);
      if (payload == reinterpret_cast<const uint8_t *>(pkthdr + 1)) {
        req_msgbuf = MsgBuffer(pkthdr, pkthdr->msg_size_);
      } else {
        req_msgbuf = MsgBuffer::make_detached(
            pkthdr, const_cast<uint8_t *>(payload), pkthdr->msg_size_);
      ...
      copy_data_to_msgbuf(&req_msgbuf, 0, pkthdr);
    }
...
#if defined(ERPC_CXL) || defined(ERPC_UB)
      printf("%s: Re-sending credit return.\n", issue_msg);
#else
...
#if defined(ERPC_CXL) || defined(ERPC_UB)
      printf("%s: Dropping because response not available yet.\n",
                  issue_msg);
#else
