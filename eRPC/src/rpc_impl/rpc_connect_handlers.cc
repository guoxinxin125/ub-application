...
void Rpc<TTr>::handle_connect_req_st(const SmPkt &sm_pkt) {
  ...
  if (std::is_same<TTr, CTransport>::value) {
    CTransport *cxl_transport = static_cast<CTransport *>(transport_);
    cxl_transport->register_rx_peer(session->client_.rpc_id_);
  }
#endif
  ...
  printf("Rpc %u: Created Server session_num = %zu for client %s from %s (rpc %u)\n",
         rpc_id_, session->server_.session_num_,
         sm_pkt.client_.name().c_str(), sm_pkt.client_.hostname_, sm_pkt.client_.rpc_id_);
