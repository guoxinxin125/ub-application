...
ClientResult run_client_pair(int pair_idx, int concurrency, size_t msg_size,
  ...
  printf("RDMA pair client %d: uri=%s, rpc_id=%u, server_rpc_id=%u\n",
         pair_idx, client_uri.c_str(), static_cast<unsigned>(client_rpc_id),
  ...
  if (session_num < 0) {
    printf("RDMA pair client %d: Failed to create session, error: %d\n",
           pair_idx, session_num);
  ...
  if (!rpc->is_connected(session_num)) {
    printf("RDMA pair client %d: Failed to connect after %d attempts\n",
           pair_idx, max_attempts);
    ready_count->fetch_add(1, std::memory_order_release);
    delete rpc;
    return result;
  }
  ...
  printf("RDMA pair client %d: avg latency %.3f us, checksum %llu\n",
         pair_idx, result.total_lat_us / num_requests,
...
int main(int argc, char **argv) {
  ...
  if (concurrency > static_cast<int>(erpc::kSessionReqWindow)) {
    printf("RDMA pair client: Requested concurrency %d, clamping to "
  ...
  if (num_requests <= 0) {
    printf("RDMA pair client: requests/pair must be positive\n");
    return -1;
  }
  if (num_pairs <= 0) num_pairs = 1;
  if (num_pairs > max_pairs) {
    printf("RDMA pair client: Requested %d pairs, clamping to %d\n",
           num_pairs, max_pairs);
    num_pairs = max_pairs;
  }

  printf("RDMA pair client: concurrency=%d, msg_size=%zu, pairs=%d, "
  ...
  if (successful_pairs != num_pairs) {
    printf("RDMA pair client: %d/%d pairs completed successfully\n",
  ...
  printf("RDMA pair client: aggregate avg latency %.3f us\n", avg_lat_us);
  printf("RDMA pair client: measured time %.6f s, throughput %.3f Mops, "
         "payload bandwidth %.3f Gbps\n",
         elapsed_sec, req_mops, payload_gbps);
  printf("RDMA pair client: aggregate response checksum %llu\n",
