...
void configure_cxl_env(int num_clients) {
#ifdef ERPC_CXL
  const std::string worker_count = std::to_string(num_clients + 1);
...
ClientResult run_client(int client_idx, int concurrency, size_t msg_size,
  ...
  printf("MC client %d: uri=%s, rpc_id=%u, server_rpc_id=%u\n", client_idx,
  ...
  if (session_num < 0) {
    printf("MC client %d: Failed to create session, error: %d\n", client_idx,
           session_num);
  ...
  if (!rpc->is_connected(session_num)) {
    printf("MC client %d: Failed to connect after %d attempts\n", client_idx,
           max_attempts);
    ready_count->fetch_add(1, std::memory_order_release);
    delete rpc;
    return result;
  ...
  printf("MC client %d: avg latency %.3f us, checksum %llu\n", client_idx,
         result.total_lat_us / num_requests,
...
int main(int argc, char **argv) {
  ...
  if (concurrency > static_cast<int>(erpc::kSessionReqWindow)) {
    printf("MC client: Requested concurrency %d, clamping to per-session "
           "request window %zu\n",
           concurrency, erpc::kSessionReqWindow);
    concurrency = static_cast<int>(erpc::kSessionReqWindow);
  }
  if (num_requests <= 0) {
    printf("MC client: requests/client must be positive\n");
    return -1;
  }
  if (num_clients <= 0) num_clients = 1;
  if (num_clients > max_clients) {
    printf("MC client: Requested %d clients, clamping to %d\n", num_clients,
           max_clients);
    num_clients = max_clients;
  }
  configure_cxl_env(num_clients);
  if (msg_size < sizeof(int)) msg_size = sizeof(int);

  printf("MC client: concurrency=%d, msg_size=%zu, clients=%d, "
  ...
  if (successful_clients != num_clients) {
    printf("MC client: %d/%d clients completed successfully\n",
  ...
  printf("MC client: aggregate avg latency %.3f us\n", avg_lat_us);
  printf("MC client: measured time %.6f s, throughput %.3f Mops, payload "
         "bandwidth %.3f Gbps\n",
         elapsed_sec, req_mops, payload_gbps);
  printf("MC client: aggregate response checksum %llu\n",
