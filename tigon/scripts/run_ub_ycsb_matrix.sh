...
if [[ $# -ne 3 ]]; then
  ...
  echo "env: TIGON_BUILD_DIR UB_PROVIDER_HOST UB_PROVIDER_NUMA UB_NUMA_NODE UB_REGION_MB UB_TIGON_*" >&2
  exit 2
fi
...
run_one() {
  ...
  echo "matrix run: id=$id mode=$mode query=$query transport=$transport_name prefix=$prefix provider_numa=${provider_numa:-auto}"
