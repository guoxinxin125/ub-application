...
usage() {
  ...
  echo "env: TIGON_BUILD_DIR UB_PROVIDER_HOST UB_PROVIDER_NUMA UB_NUMA_NODE UB_REGION_MB UB_RESULT_DIR UB_TIGON_* UB_TPCC_*" >&2
}
[[ $# -eq 3 ]] || { usage; exit 2; }
id=$1
servers=$2
...
binary=${TIGON_BUILD_DIR:-$tigon_dir/build-ub}/bench_tpcc
...
partitions=${UB_TIGON_PARTITIONS:-$((2 * threads))}
...
for pair in "region_mb:$region_mb" "threads:$threads" "partitions:$partitions" "run_seconds:$run_seconds" "warmup_seconds:$warmup_seconds"; do require_uint "${pair%%:*}" "${pair#*:}"; done
(( region_mb > 0 && threads > 0 && partitions > 0 && run_seconds > 0 )) || { echo "region_mb, threads, partitions and run_seconds must be positive" >&2; exit 2; }
...
require_percent neworder_dist "$neworder_dist"
require_percent payment_dist "$payment_dist"
...
run_one() {
  ...
  local meta="TIGON_RUN_META system=ub workload=tpcc run_id=$run_id host_id=$id repetition=$repetition mode=$mode query=$query transport=$transport_name partitions=$partitions workers=$threads keys= rw_ratio= zipf= cross_ratio= neworder_dist=$neworder_dist payment_dist=$payment_dist warmup_seconds=$warmup_seconds run_seconds=$measurement_seconds total_seconds=$run_seconds logging=off"
  ...
  if [[ "$dry_run" == 1 ]]; then
    ...
    mkdir -p "$result_dir"
    local log="$result_dir/ub_tpcc_${run_id}_host${id}_${mode}_${query}_${transport_name}.log"
    set +e
